package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit
   {
      ...
      private function createView() : void
      {
         ...
         _view.visible = false;
         var_311 = _view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(var_303 && var_1268)
         {
            _loc5_ = var_29.avatarRenderManager.createAvatarImage(figure,AvatarScaleType.SMALL,var_602);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_41,parseInt(FigureData.const_603));
         ...
         if(var_311)
         {
            if(false)
            {
               var_311.bitmap.dispose();
            }
            var_311.bitmap = new BitmapData(var_311.width,var_311.height,true,0);
            _loc7_ = (0 - _loc1_.width) / 2;
            _loc8_ = (0 - _loc1_.height) / 2;
            var_311.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               var_303 = var_29.figureData.getFigureString();
               var_602 = var_29.gender;
               var_29.handler.saveWardrobeOutfit(var_1519,this);
               updateView();
               break;
            case "get_button":
               if(var_303)
               {
                  var_29.loadAvatarInEditor(var_303,var_602,var_29.clubMemberLevel);
               }
         }
      }
   }
